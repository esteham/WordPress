...
add_action('wp_ajax_pcc_update_category', 'pcc_update_category');
...
function pcc_get_categories() 
{
    ...
    foreach ($categories as $index => $cat) 
    {
        echo "<tr>
            ...
            <td><button class='btn btn-sm btn-info edit-category' data-id='{$cat->id}' data-name=\"".esc_attr($cat->category_name). "\">Edit</button></td>
...
function pcc_update_category()
{
    global $wpdb;
    $table = $wpdb->prefix . 'pcc_categories';
    $id = intval($_POST['cat_id']);
    $name = sanitize_text_field( $_POST['category_name']);
    $wpdb->update($table, ['category_name'=>$name],['id'=>$id]);
