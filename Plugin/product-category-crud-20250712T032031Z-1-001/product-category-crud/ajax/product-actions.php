    ...
    foreach ($products as $index => $p) 
    {
        echo "<tr>
            ...
            <td><img src='{$p->image}' width='50'></td>
             ...
             <td><button class='btn btn-sm btn-info edit-product' data-id='{$p->id}' data-name=\"".esc_attr($p->product_name)."\" data-description=\"".esc_attr($p->description)."\"
                 data-unit='{$p->unit_price}'
                 data-sell='{$p->selling_price}'
                 data-stock='{$p->total_stock}'
                 data-category='{$p->category_id}'>Edit</button></td>
...
add_action('wp_ajax_pcc_update_product', 'pcc_update_product');

function pcc_update_product()
{
    global $wpdb;
    ...
    if(!empty($_FILES['image']['name']))
    {
        ...
        if(move_uploaded_file($file['tmp_name'], $destination))
        {
            $data['image'] = plugin_dir_url(__DIR__).'uploads/'.$filename;
        }
    }
